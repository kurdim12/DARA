...
**Not run yet.** This file is written by the eval harness, against a deployed
URL, because the API key lives only in the Worker.

```bash
npm run eval -- --target https://dara.<your-subdomain>.workers.dev --compare
```

That runs all 22 golden cases on `claude-sonnet-5` and on `claude-haiku-4-5`,
